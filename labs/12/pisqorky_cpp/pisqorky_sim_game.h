...
typedef std::vector<std::tuple<Pisqorky, Policy, float>> History;

typedef std::vector<std::tuple<const Pisqorky*, Policy*, float*>> Batch;
...
void worker_evaluator(const Pisqorky& game, Policy& policy, float& value) {
